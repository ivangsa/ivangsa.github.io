...
An API contract is the densest context you have about a system, and in the agentic era this is even more important. In a handful of files it says what the system does, what it accepts and returns, what it publishes, and what it promises to keep stable, and it says all of it in a form that is precise and machine readable. Little else you publish concentrates that much meaning in so little space, which is exactly why it deserves to be treated as a product rather than as a file that falls out of the build.

But ZenWave Domain Model Language, ZDL, is denser still than the API contracts, because it is designed to be very compact and it carries both the internal model and the external contract in one file.

In Arcadia Editions the API is not written by hand. [OpenAPI and AsyncAPI are derived from the ZDL domain model](/articles/arcadia/009-dsl-modeling-for-apis-generate-openapi-and-asyncapi-from-zdl), so the model carries even more meaning than either contract does, because both specifications are only projections of it. Every organization already stands on APIs and their specifications, which is the common ground this article assumes, and some will want to go one step further, to the model those specifications come from. Either way the order is the same. The model comes first, the contract is its published projection, and that hierarchy is the spine of everything below.


## Why treat the contract as a product?

What makes an API a product is that it has its own customers. Sometimes those customers are external developers integrating with your platform, more often they are other teams inside your own company, and increasingly they are AI coding agents generating clients, tests and whole applications from a machine readable spec.
